...
class OutputWriterTest < Minitest::Test
  def test_compact_verified_output_round_trips_exactly
    Dir.mktmpdir("pulseproof-output-writer-") do |directory|
      ...
      PulseProof::OutputWriter.json(path, payload, pretty: false, verify: true)

      assert_equal payload, JSON.parse(File.read(path, encoding: "UTF-8"))
      refute_includes File.read(path, encoding: "UTF-8"), "\n  "
